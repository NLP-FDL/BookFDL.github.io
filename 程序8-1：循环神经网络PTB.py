...
#pip install tensorflow==1.15.0 在anaconda 虚拟环境下
...
DATA_PATH='data' #data目录中应该预先存放本程序中要用到的语料，即PTB(Penn Treebank Dataset)数据集，也就是宾州树库数据集 。PTB数据集是语言模型研究常用的数据集。其下载地址在 Tomas Mikolov的主页： http://www.fit.vutbr.cz/~imikolov/rnnlm/simple-examples.tgz。解压后，将其中的整个data文件夹拷贝到当前目录即可。数据集共有9998个单词，加上稀有词语的特殊符号<unk>和语句的结束标记，共有10000个单词。
